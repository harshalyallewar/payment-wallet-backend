...
public interface DailyUserSummaryRepository extends JpaRepository<DailyUserSummary, Long> {
    ...
    @Modifying
    @Query(value = """
        INSERT INTO daily_user_summary (user_id, date, total_credits, net_change, last_updated)
        VALUES (?1, ?2, ?3, ?3, NOW())
    ...
    @Modifying
    @Query(value = """
        INSERT INTO daily_user_summary (user_id, date, total_debits, net_change, last_updated)
    ...
    @Modifying
    @Query(value = """
        INSERT INTO daily_user_summary (user_id, date, failed_txns, last_updated)
        VALUES (?1, ?2, 1, NOW())
        ON CONFLICT (user_id, date) DO UPDATE SET
